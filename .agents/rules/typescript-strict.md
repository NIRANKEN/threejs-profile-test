...
AIペアプログラミングの推論精度およびコードベースの堅牢性を最大化するため、極めて厳密な型定義を徹底します。
...
`unknown` 型の変数を操作する場合、必ず事前に「型ガード（Type Guards）」を用いて安全に型を絞り込んでください。
