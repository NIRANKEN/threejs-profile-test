...
// ❌ 参照型の再生成（GCのスパイク・ガベージコレクション負荷の原因）
useFrame(() => {
...
同じオブジェクト（木、岩、パーティクルなど）を複数描画する場合は、常に `InstancedMesh` または `BatchedMesh` の使用を検討してください。`mesh` を `map` 関数で大量にループさせることは深刻なドローコール増加を招きます。

## 4. イベント駆動のレンダリング

常時アニメーションが不要なシーンでは、`<Canvas frameloop="demand">` を設定し、`invalidate()` で必要な時のみ再描画を行うようにして電力とリソース消費を削減します。
